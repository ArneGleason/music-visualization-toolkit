...
sc.collection.objects.link(cam);cam.location=(0,0,10);cam_data.type='ORTHO';cam_data.ortho_scale=16;sc.camera=cam
...
em=n.new('ShaderNodeEmission');em.inputs['Strength'].default_value=1.1
...
for j in range(900):
    sx=rng.uniform(.06,.38) if j%2 else rng.uniform(.64,.96)
    sy=rng.uniform(.34,.93);radius=rng.uniform(.007,.018)
    ...
    for k in range(8):
        ...
        verts.append(((sx-.5)*16+dx,(.5-sy)*9+dy,0))
        ends.append(((ex-.5)*16+dx,(.5-ey)*9+dy,0))
    faces.append(tuple(range(base,base+8)));colors.append(color)
mesh=bpy.data.meshes.new('900 motes');mesh.from_pydata(verts,[],faces);mesh.materials.append(mat)
...
plate=nodes.new('CompositorNodeImage');plate.image=bpy.data.images.load(str(OUT/'plate/0001.png'))
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'shot8.blend'))
for frame in range(1,23):
    sc.frame_set(frame)
    plate.image=bpy.data.images.load(str(OUT/'plate'/f'{frame:04}.png'),check_existing=True)
    sc.render.filepath=str(OUT/'frames'/f'{frame:04}.png')
