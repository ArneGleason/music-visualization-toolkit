...
for family,n in [('pieces',48),('meant',11)]:
    for width in [1280,1920]:
        ...
        if missing:
            with (O/f'{family}_{width}_queue.log').open('w') as log:
                p=subprocess.run([BL,'-b','--python-exit-code','1','-P',str(O/'inserts_native.py'),'--',family,str(width),','.join(map(str,missing))],cwd=R,stdout=log,stderr=subprocess.STDOUT)
            status='rendered' if p.returncode==0 else 'failed'
        else:status='existing'
        records.append({'family':family,'width':width,'status':status});(O/'batch3_insert_queue.json').write_text(json.dumps(records,indent=2));print(family,width,status,flush=True)
