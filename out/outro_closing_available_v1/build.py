...
A=load('out/outro_closing_generations_v1/A3/synced.mp4');B=load('out/outro_closing_generations_v1/B3/base.mp4')
...
for song in range(4362,4854):
    if song<4504:
        ...
        f=close(A[song-4492],1.09+.035*smooth(4504,4557,song),600,345)
        label='A3 / returned Kling / closer framing'
    elif song<4574:
        ...
        f=close(A[song-4492],1.125,600,345)
        label='A3 / returned Kling / natural reach retained'
    elif song<4644:
        f=close(B[song-4500],1.08,640,345)
        label='B3 take 2 / Flow performance only / NOT lip-synced'
    elif song<4671:
        f=A[song-4492].copy()
        # Approximate CRT shutdown on old stand-in screen; no physical hand action.
        off=smooth(4661,4670,song)
        screen=np.exp(-((x-1200)/150)**8-((y-460)/130)**8)
        ...
        label='A3 / returned Kling and switch gesture / shutdown sketch'
    elif song<4689:
        f=B[song-4500].copy()
        off=smooth(4681,4688,song)
        f=(f*(1-.64*off)).astype(np.uint8)
        label='B3 take 2 / unsynced final Night / local dimming sketch'
    elif song<4782:
