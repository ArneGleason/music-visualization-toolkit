...
original=ROOT/'out/outro_B1_v2/synced.mp4'
assert hashlib.sha256(original.read_bytes()).hexdigest()=='7055e5d5037286e56692ddc33343f551e4d5aa212f14802e2ca181756ddf7ea6'
...
pcm=subprocess.check_output(['ffmpeg','-v','error','-i',str(ROOT/'out/outro_B1_v2/guide.wav'),'-ac','1','-ar','24000','-f','f32le','-'])
...
energy=np.clip(energy/max(np.percentile(energy[67:97],90),1e-6),0,1)
def smooth(a,b,x):
    p=np.clip((x-a)/(b-a),0,1);return p*p*(3-2*p)
gate=smooth(61,67,np.arange(N))*(1-smooth(96,101,np.arange(N)))
energy*=gate
...
for i in range(1,N):
    ...
    if np.linalg.norm(delta)>16:raise RuntimeError(f'Unreasonable artifact motion {i}: {delta}')
...
for i,f in enumerate(frames):
    t=i/24;cx,cy=centers[i];v=float(energy[i]);pic=f.astype(np.float32)/255
    ...
    records.append({'source_frame':i,'song_frame':4165+i,'center':centers[i].tolist(),'voice_envelope':v})
(OUT/'settings.json').write_text(json.dumps({'source':source,'frames':N,'fps':24,'song_origin':4165,'tracking':'artifact local sparse optical flow, forward/backward rejection, translation only','driver':'local90percentile of Define there, gated to own phrase with release; not a speaker-separated stem','records':records},indent=2))
