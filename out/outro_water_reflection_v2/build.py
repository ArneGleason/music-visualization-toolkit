...
for song in range(4362,4854):
    ...
    if num in (1,3):f=tv(A2[song-4320].copy(),2,song)
        ...
        warp=5*shrink*np.sin(y*.09+t*1.2)+3*np.sin(y*.21-t*.8)
        footprint=np.exp(-((dx-warp)/(7+18*shrink))**2-((dy)/(20+112*shrink))**2)
        # Advected multi-scale noise: irregular glints rather than periodic rungs.
        rng=np.random.default_rng(731)
        noise=np.zeros_like(x)
        for gh,gw,weight in [(45,64,.5),(120,96,.3),(240,160,.2)]:
            tile=rng.random((gh,gw)).astype(np.float32)
            expanded=cv2.resize(tile,(1280,720),interpolation=cv2.INTER_CUBIC)
            noise+=weight*cv2.remap(expanded,(x+9*np.sin(y*.027+t*.6)).astype(np.float32),
                (y+t*8).astype(np.float32),cv2.INTER_LINEAR,borderMode=cv2.BORDER_REFLECT)
        bands=np.clip((noise-.42)*3,0,1)**2
        bands=cv2.GaussianBlur(bands,(0,0),3.5)
        breakup=.3+.7*(.5+.5*np.sin(x*.09+y*.11-t*.9))
        strength=footprint*bands*breakup*wm*(.25+.6*shrink)
        reflection=strength[:,:,None]*np.array([75,185,100])
        reflection=cv2.GaussianBlur(reflection,(0,0),1.8)
        reflection+=cv2.GaussianBlur(reflection,(0,0),6)*.4
        f=light(f,reflection*wm[:,:,None])
        # Nine subframe shutter samples, reduced loops, softly focused body.
        ...
        for dt in np.linspace(-.4,.4,9):
            ...
            sample=sprite(np.zeros_like(f),bx,by,2+23*ss,t+dt/24,(1-us)*1.4+.1)
            beam+=sample.astype(np.float32)/9
        beam=cv2.GaussianBlur(beam,(0,0),1.0)
        f=light(f,beam)
