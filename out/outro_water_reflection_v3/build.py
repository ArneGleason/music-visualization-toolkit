...
w.release()
subprocess.run(['ffmpeg','-v','error','-n','-i',str(OUT/'silent.mp4'),'-i',str(ROOT/'audio/song.wav'),'-filter_complex',f'[1:a]atrim=start={4710/24}:end={4782/24},asetpts=PTS-STARTPTS[a]','-map','0:v','-map','[a]','-t','3','-c:v','libx264','-crf','18','-c:a','aac','-movflags','+faststart',str(OUT/'water_preview.mp4')],check=True)
subprocess.run(['ffmpeg','-v','error','-i',str(OUT/'water_preview.mp4'),'-f','null','-'],check=True)
(OUT/'cut_map.json').write_text(json.dumps(rows,indent=2));print('72-frame water-only review ready; audio origin4710, original iris retained.')
