...
def main():
    ...
    guide = OUT / 'guide_A1_aligned.wav'
    ...
    report = {'guide_samples':len(x), 'embedded_samples':len(y),
        ...
        'best_lag_correlation':score, 'song_origin_frame':4165, 'fps':24,
        'silent_interval_2_1_to_3_6_peak':float(np.max(abs(y[100800:172800])))}
    ...
    graph = ('[0:v]setpts=PTS-STARTPTS[v];[1:a]'
        'atrim=start=173.541666666667:end=181.541666666667,asetpts=PTS-STARTPTS[a]')
    ...
    run(['ffmpeg','-v','error','-y','-i',synced,'-vf',
        'fps=2,scale=640:360,tile=4x4','-frames:v','1',OUT/'synced_contact.jpg'])
    edl_path = ROOT/'shots/shotlist.json'
    before = hashlib.sha256(edl_path.read_bytes()).hexdigest()
    edl = json.loads(edl_path.read_text())
    for shot in edl['shots']:
        if shot['id'] in ('o02','o04'):
            shot['clip'] = dict(clip, in_sec=(round(shot['start_sec']*24)-4165)/24)
        elif shot['id'] == 'o03':
            # Existing other speaker retains the previously verified stem fix.
            shot['clip']['in_sec'] = (round(shot['start_sec']*24)-4169)/24
    ...
    with (OUT/'assembly.log').open('w') as log:
        subprocess.run([sys.executable,'tools/blender_comp.py','--proxy',
            '--start','4177','--end','4330','--shotlist',str(snap),
            '--lyric-flat','shots/lyric_motion_full.json','--out',str(raw)],
            cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,check=True)
    graph = ('[0:v]setpts=PTS-STARTPTS[v];[1:a]'
        'atrim=start=174.041666666667:end=180.458333333333,asetpts=PTS-STARTPTS[a]')
    run(['ffmpeg','-v','error','-y','-i',raw,'-i',ROOT/'audio/song.wav',
        '-filter_complex',graph,'-map','[v]','-map','[a]','-frames:v','154',
