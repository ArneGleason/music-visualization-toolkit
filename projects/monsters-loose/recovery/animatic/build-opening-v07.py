...
s.frame_start=1;s.frame_end=int(nextshot.frame_final_end)-1
s.name='Monsters Loose - approved opening assembly v07'
...
for st in seq:
 if st.type=='TEXT' and 207<=st.frame_final_start<504:
  ...
  if st.channel==101:st.text='Harper welcomes another night on duty, then notices the fallen bolt.'
  if st.channel==102:st.text='She crouches to inspect the open gate: "Oh my."'
...
for frame in (260,343,550):
 s.frame_set(frame);s.render.filepath=str(root/f'v07-check-{frame}.png');bpy.ops.render.render(write_still=True)
s.render.image_settings.media_type='VIDEO';s.render.image_settings.file_format='FFMPEG'
s.render.filepath=str(root/'MonstersLoose-v07-opening-through-sign.mp4')
s.frame_set(1);bpy.ops.file.make_paths_relative()
bpy.ops.wm.save_as_mainfile(filepath=str(root/'MonstersLoose-v07-opening-through-sign.blend'))
