...
for st in list(seq):
    if st.channel==1 and st.frame_final_start in (1033,1103,1162):seq.remove(st)
...
for st in seq:
    if st.type=='TEXT' and st.frame_final_start==504:
        ...
        if st.channel==102:st.text='Three creatures stir; a distant steel gate rattles to the guitar impacts.'
for st in seq:
    if st.type=='TEXT' and 'its full scale stays out of view' in st.text:
        st.text='Harper follows the gate upward, revealing its monumental height.'
...
s.name='Monsters Loose Arctic announcement review v22'
s.render.image_settings.media_type='VIDEO'
s.render.image_settings.file_format='FFMPEG'
s.render.filepath=str(o/'MonstersLoose-v22-opening-through-lab.mp4')
bpy.ops.file.make_paths_relative()
bpy.ops.wm.save_as_mainfile(filepath=str(o/'MonstersLoose-v22-arctic.blend'))
