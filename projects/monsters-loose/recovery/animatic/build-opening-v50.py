...
bpy.ops.wm.open_mainfile(filepath=str(o/'MonstersLoose-v49-through-habitats.blend'))
s=bpy.context.scene;seq=s.sequence_editor.strips
for st in list(seq):
 if st.channel==1 and st.frame_final_start==2092:seq.remove(st)
for sh in json.loads((r/'shots/shotlist.json').read_text())['shots']:
 if not sh['id'].startswith('CREATURE-PORTRAIT-'):continue
...
s.frame_start=1;s.frame_end=2461;s.name='Monsters Loose v50 three creature portraits'
s.render.filepath=str(o/'MonstersLoose-v50-opening-through-habitats.mp4')
bpy.ops.file.make_paths_relative();bpy.ops.wm.save_as_mainfile(filepath=str(o/'MonstersLoose-v50-through-habitats.blend'));bpy.ops.render.render(animation=True)
