...
s.name='Monsters Loose - approved opening assembly v06'
...
for frame in (260,343,550):
 s.frame_set(frame);s.render.filepath=str(root/f'v06-check-{frame}.png');bpy.ops.render.render(write_still=True)
s.render.image_settings.media_type='VIDEO';s.render.image_settings.file_format='FFMPEG'
s.render.filepath=str(root/'MonstersLoose-v06-opening-through-sign.mp4')
s.frame_set(1);bpy.ops.file.make_paths_relative()
bpy.ops.wm.save_as_mainfile(filepath=str(root/'MonstersLoose-v06-opening-through-sign.blend'))
