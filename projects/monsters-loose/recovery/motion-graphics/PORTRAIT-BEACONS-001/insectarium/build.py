...
s.render.fps=24;s.frame_start=1;s.frame_end=26
...
for i,rows in tracks.items():
 ...
 mute.scale=(10,10,1);ma.default_value=.97
 halo,ha=patch('Beacon %s bloom'%i,(1,.014,.002));halo.scale=(10,10,1)
 core,ca=patch('Beacon %s red lamp'%i,(1,.025,.004));core.scale=(2,2,1)
 hot,wa=patch('Beacon %s hot center'%i,(1,.35,.14));hot.scale=(.85,.85,1)
 for f,(x,y,bg) in enumerate(rows,1):
  ...
  ma.default_value=.97*visible;ma.keyframe_insert(data_path='default_value',frame=f)
  p=math.exp(-.5*(d/2.4)**2)*visible
...
bpy.ops.wm.save_as_mainfile(filepath=str(o/'insectarium-beacon-v001.blend'))
