...
s.render.fps=24;s.frame_start=1;s.frame_end=145
...
for i,rows in tracks.items():
 ...
 mute.scale=(7,7,1);ma.default_value=0
 halo,ha=patch('Beacon %s bloom'%i,(1,.014,.002));halo.scale=(7,7,1)
 ...
 for f,(x,y,bg) in enumerate(rows,1):
  ...
  ma.default_value=0*visible;ma.keyframe_insert(data_path='default_value',frame=f)
...
bpy.ops.wm.save_as_mainfile(filepath=str(o/'Habitat-lights-v001.blend'))
