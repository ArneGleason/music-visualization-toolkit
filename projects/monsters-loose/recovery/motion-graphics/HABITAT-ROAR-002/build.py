...
m=bpy.data.materials.new('Existing lit habitat movie');m.use_nodes=True;n=m.node_tree.nodes;n.clear();tex=n.new('ShaderNodeTexImage');tex.image=bpy.data.images.load(str(o.parent/'HABITAT-ROAR-001/source/frame-0001.png'));tex.image.source='SEQUENCE';tex.image_user.frame_duration=56;tex.image_user.frame_start=1;tex.image_user.use_auto_refresh=True;em=n.new('ShaderNodeEmission');out=n.new('ShaderNodeOutputMaterial');m.node_tree.links.new(tex.outputs['Color'],em.inputs['Color']);m.node_tree.links.new(em.outputs[0],out.inputs['Surface']);me.materials.append(m)
...
for rec in records:
 ...
 for v,(x,y,z) in zip(key.data,points):
  ...
  for start in starts:
   age=f-start
   if 0<=age<22:
    band=(radius-(10+age*8.5))/9;strength+=19.0*math.exp(-.5*band*band)*math.cos(band*1.8)*(1-age/26)
...
def seq(name,folder,prefix,color):
 n=nodes.new('CompositorNodeImage');n.name=name;n.image=bpy.data.images.load(str((o.parent/'HABITAT-ROAR-001' if folder in ['source','masks'] else o)/folder/(prefix+'0001.png')));n.image.source='SEQUENCE';n.image.colorspace_settings.name=color;n.frame_duration=56;n.frame_start=1;n.use_auto_refresh=True;return n
...
(o/'render').mkdir(exist_ok=True);s.render.filepath=str(o/'render/frame-');bpy.ops.wm.save_as_mainfile(filepath=str(o/'Habitat-enclosed-roar-v002.blend'),compress=True);bpy.ops.render.render(animation=True)
