...
reaction=o/'Harper-reaction-announcement-v001.mp4'
...
run([*inputs,'-filter_complex',graph,'-map','[v]','-map','[audio]','-c:v','libx264','-crf','17','-preset','fast','-pix_fmt','yuv420p','-c:a','aac','-b:a','320k','-movflags','+faststart',str(o/'Arctic-patrol-announcement-reaction-v001.mp4')])

for name,end in [('KLING-PATROL-001',151),('KLING-REACTION-001',171)]:
...
new=[
    ...
    dict(id='HARPER-ARCTIC-REACTION',start_frame=874,end_frame_exclusive=1033,source=str(reaction.relative_to(r)).replace('\\','/'),source_in_frame=12,assumed_pre_roll_frames=12,status='generated_with_announcement_for_review',timing_record='video-tests/KLING-REACTION-001/timing.json')
]
...
d['planned_scene3_revision']['preview']='motion-graphics/ARCTIC-PA-001/Arctic-patrol-announcement-reaction-v001.mp4'
p.write_text(json.dumps(d,indent=2)+'\n')
(o/'timing.json').write_text(json.dumps({
    ...
    'credits':{'patrol':56,'reaction':64,'total':120}
},indent=2)+'\n')
