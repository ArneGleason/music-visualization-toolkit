...
grid=json.loads((job/'animatic'/'animatic-data.json').read_text())['beats']
times=[b['time'] for b in grid]
source_zero_master_seconds=367/24
# One bob cycle per two beats; lateral sway and roll take four beats.
# Interpolate the existing DAW-derived grid, retaining its changing beat lengths.
for f in range(1,194):
 t=(f-1)/24
 master_time=source_zero_master_seconds+t
 i=max(0,min(len(grid)-2,bisect.bisect_right(times,master_time)-1))
 frac=(master_time-times[i])/(times[i+1]-times[i])
 beat=grid[i]['project_beat']+frac*(grid[i+1]['project_beat']-grid[i]['project_beat'])
 phase=math.pi*(beat-40)
 # Uneven but restrained walking rhythm with slow lateral breathing.
 values={'X':5.5*math.sin(phase*.5)+1.5*math.sin(phase*.25),
         'Y':7.5*math.sin(phase)+.8*math.sin(phase*2+.6),
...
lens=g.nodes.new('CompositorNodeLensdist');lens.label='Stronger peripheral curvature, very slight color separation'
...
s.frame_set(70);s.render.filepath=str(root/'pov-effect-v002-check.png');bpy.ops.render.render(write_still=True)
...
s.render.filepath=str(root/'Stealth-POV-motion-lens-v002.mp4');s.frame_set(1)
bpy.ops.wm.save_as_mainfile(filepath=str(root/'Stealth-POV-motion-lens-v002.blend'))
