...
s.render.engine='BLENDER_WORKBENCH';s.render.resolution_x=1280;s.render.resolution_y=720;s.render.resolution_percentage=100;s.render.fps=24;s.frame_start=1;s.frame_end=108;s.render.film_transparent=True
...
for i,(lines,start,end) in enumerate([(['NO WAY!'],1,39),(['YES WAY!'],49,96)]):
 cx,cy,sx,sy,tilt=[(5.0,2.7,.62,.68,0),(5.0,-1.9,.68,.68,0)][i]
 words_by_line=[]
 for line in lines:
  row=[]
  for text in line.split():
   cu=bpy.data.curves.new(text,'FONT');cu.body=text;cu.font=font;cu.align_x='LEFT';cu.align_y='CENTER';cu.size=1.1
 ...
 initial_sx=max(.25,(first_width+.9)/6.3)
 def shape(f):
  gx=smooth((f-start-5)/7) if i==0 else smooth((f-start-10)/5);gy=gx
  ...
  tip=(.65,-.10) if i==0 else (2.50,.80)
  pts=[]
  for x,y in points:
   w=max(0,1-math.hypot(x+4,y+1.48)/2.05)**1.5
   px=cx+dx+x*ax;py=cy+dy+(-y if i==1 else y)*ay
   px+=(tip[0]-(cx+dx-4*ax))*w;py+=(tip[1]-(cy+dy+(1.48 if i==1 else -1.48)*ay))*w
   pts.append((px,py))
 ...
 for li,row in enumerate(words_by_line):
  ...
  for wi,(ob,w) in enumerate(row):
   arrival=start+(wi*7 if i==0 else wi*12);visible(ob,arrival,end)
   for f in range(1,109):
    pts,bx,by,gx,gy=shape(f)
    xx=(-first_width/2+(offset+first_width/2)*gx) if li==0 and wi==0 else offset
    ob.location=(bx+xx,by,.07);ob.keyframe_insert(data_path='location',frame=f)
   offset+=w+.18
...
s.render.image_settings.file_format='PNG';s.render.image_settings.color_mode='RGBA';s.render.filepath=str(o/'bubbles/bubble-')
(o/'bubbles').mkdir(exist_ok=True)
bpy.ops.wm.save_as_mainfile(filepath=str(o/'Exchange-bubbles-v001.blend'));bpy.ops.render.render(animation=True)
