# Prompts

41 setups covering 59 cuts. Generate the still first (free), approve it, then the video from it (Frames-to-Video, start frame = the still). Longest cut using each setup is noted -- generate at least that long.

**Cast lines** (substituted verbatim wherever a prompt says `{{name}}`):

- `astronaut` — the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs
- `astronomer` — the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt
- `suit` — her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned
- `suit_field` — her silver suit, carrying a small brass hand lantern
- `suit_orbital` — her silver suit with a clear domed glass helmet sealed to the collar ring
- `suit_liner` — her silver suit with the collar ring removed, a plain open neckline
- `merfolk` — the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison

**Reference images** (attach the matching ones every time):

- `astronaut_likeness` — refs/astronaut_likeness.jpg
- `specimen_ref` — refs/specimen_ref.jpg
- `merfolk_likeness` — refs/merfolk_likeness.jpg
- `merfolk_likeness_gpt` — refs/merfolk_likeness_gpt.jpg
- `world_anchor` — refs/world_anchor.jpg
- `character_master` — refs/character_master.png
- `astronaut_front` — refs/astronaut_front.jpg
- `astronaut_close` — refs/astronaut_close.png
- `astronaut_move` — refs/astronaut_move.png
- `astronomer_front` — refs/astronomer_front.jpg
- `astronomer_close` — refs/astronomer_close.png
- `suit_turnaround` — refs/suit_turnaround.png
- `merfolk_sheet` — refs/merfolk_sheet.png
- `palette` — refs/palette.png
- `mars_world` — refs/Rivers-of-Mars.png
- `world_basin` — refs/world_basin.png
- `world_forest` — refs/world_forest.png
- `world_orbit` — refs/world_orbit.png

---

## `obs_dome_open`  ·  concept  ·  Observatory dome opens

Used by 1 cut(s): s001 (2.9s)
Sections: scene-01 · **generate ≥ 2.9s**
Conventions: `CAM-APPROACH`, `FRAME-CENTRIPETAL`, `ANIM-SLOW-IN-OUT`

**Still**

```
Film still. Night. Symmetrical frontal wide of a hillside observatory: the dome slit stands open and a blade of warm gold light escapes it; one bright red point - Mars - sits dead center above the dome. No people. Framing: curves, gazes and paths all leading inward to one central event.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Symmetrical frontal wide of a hillside observatory: the dome slit stands open and a blade of warm gold light escapes it; one bright red point - Mars - sits dead center above the dome. No people. The slit rumbles wider as the camera pushes slowly toward it until the glow fills the frame. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; moves ease in and ease out, quick through the middle, settling at the pose.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `obs_console_macro`  ·  concept  ·  Console and oscilloscope

Used by 1 cut(s): s002 (2.9s)
Sections: scene-01 · **generate ≥ 2.9s**
Conventions: `CAM-PROCESSION`, `FRAME-CENTRIFUGAL`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Macro across a brass-and-bakelite observatory console: trembling dials, punched tape, glowing valves, and a round oscilloscope tube drawing a warm gold waveform, centered. No people. Framing: major shapes cropped by the frame edges, the action plainly continuing beyond the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Macro across a brass-and-bakelite observatory console: trembling dials, punched tape, glowing valves, and a round oscilloscope tube drawing a warm gold waveform, centered. No people. The waveform pulses like a voice; slow lateral drift ending centered on the scope screen. Camera and motion: a lateral tracking shot, new elements entering at one frame edge as the last leave the other; elements keep their trajectories before entering and after leaving the frame; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `astronomer_desk_sync`  ·  performance  ·  Astronomer sings at desk

Used by 1 cut(s): s003 (5.7s)
Sections: signal-exchange · **generate ≥ 5.7s**
Lyric: *Hey, I need you for something. / It's important.*
Conventions: `CAM-LOCKED`, `PERF-HYSTERETIC-PRESENCE`, `FRAME-SCALE-BILLING`

**Still**

```
Film still. Medium frontal shot, perfectly centered: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt at a desk laid with specimen trays, translucent organic fragments and spectrogram strips, a brass telescope behind her, warm tungsten pools in darkness. She faces the lens, mouth open mid-phrase, deadpan self-assurance. Framing: the lead largest and nearest, everything else smaller and farther.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Medium frontal shot, perfectly centered: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt at a desk laid with specimen trays, translucent organic fragments and spectrogram strips, a brass telescope behind her, warm tungsten pools in darkness. She faces the lens, mouth open mid-phrase, deadpan self-assurance. She sings straight to camera with confident, punchy delivery - words clearly articulated, eyes locked on the lens, head steady, a subtle groove in her shoulders on the beat. Locked off. Camera and motion: locked off on a tripod, the frame never moves; she rises fast with the voice, holds across the gaps, and only slowly gives the phrase up; the lead advances and grows on the strong syllables and eases back on the breath.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `canal_map_macro`  ·  concept  ·  Specimen pieces

Used by 3 cut(s): s004 (5.7s), s006 (2.9s), s008 (2.9s)
Sections: scene-03, signal-exchange · **generate ≥ 5.7s**
Lyric: *More than that sounds.*
Conventions: `CAM-APPROACH`, `PERF-RESPONSE-CHAIN`, `ANIM-SLOW-IN-OUT`

**Still**

```
Film still. Top-down overhead macro on a leather desktop: fragments of an alien specimen laid out like a puzzle - translucent organic shards with a helical, DNA-like structure visible inside, edges that almost interlock - beside a brass loupe and a spectrogram strip; tweed-cuffed hands hold two fragments whose helices almost line up. Only the hands are in frame; no people.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Top-down overhead macro on a leather desktop: fragments of an alien specimen laid out like a puzzle - translucent organic shards with a helical, DNA-like structure visible inside, edges that almost interlock - beside a brass loupe and a spectrogram strip; tweed-cuffed hands hold two fragments whose helices almost line up. Only the hands are in frame; no people. The hands slide the fragments together; the helices almost meet, then miss by a hair, again and again. Slow overhead push-in. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; one thing initiates, another accommodates, the contact gives each an unequal recoil; moves ease in and ease out, quick through the middle, settling at the pose.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `astronomer_close_sync`  ·  performance  ·  Astronomer close-up

Used by 2 cut(s): s005 (2.9s), s007 (2.9s)
Sections: scene-03 · **generate ≥ 2.9s**
Lyric: *Things that almost fit.*
Conventions: `CAM-LOCKED`, `PERF-EXAGGERATED-ENUNCIATION`, `ANIM-MOVING-HOLD`

**Still**

```
Film still. Frontal close-up of the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt against a plain warm backdrop, eyes locked on the lens, mouth open mid-phrase, deadpan self-assurance with a flicker of hope.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Frontal close-up of the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt against a plain warm backdrop, eyes locked on the lens, mouth open mid-phrase, deadpan self-assurance with a flicker of hope. She sings with confident punch, words clearly articulated, head steady. Locked off. Camera and motion: locked off on a tripod, the frame never moves; the mouth and the head lean overshoot on the strong syllables, then damp; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `signal_crossing_space`  ·  concept  ·  Signal crosses space

Used by 2 cut(s): s009 (2.9s), s011 (2.8s)
Sections: scene-04 · **generate ≥ 2.9s**
Lyric: *Can I send you my stuff?*
Conventions: `CAM-PROCESSION`, `DEPTH-RELATIVE-SCALE`, `FRAME-CENTRIFUGAL`

**Still**

```
Film still. From orbit: a single gold pulse of light crossing a deep star field toward the coral-red globe of Mars, threaded with winding blue-green rivers and dark oases; a striped gas giant and several small moons hang in matte-painting depth beyond. No people. Framing: the same motif repeated at several sizes into the distance; major shapes cropped by the frame edges, the action plainly continuing beyond the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
From orbit: a single gold pulse of light crossing a deep star field toward the coral-red globe of Mars, threaded with winding blue-green rivers and dark oases; a striped gas giant and several small moons hang in matte-painting depth beyond. No people. The pulse travels across frame and arrives over the limb of the planet. Slow lateral drift following the light. Camera and motion: a lateral tracking shot, new elements entering at one frame edge as the last leave the other; elements keep their trajectories before entering and after leaving the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `receiver_dial_macro`  ·  concept  ·  Receiver dials tremble

Used by 1 cut(s): s010 (2.9s)
Sections: scene-04 · **generate ≥ 2.9s**
Lyric: *Tell me what you got.*
Conventions: `CAM-LOCKED`, `ANIM-MOVING-HOLD`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Night. Macro on Martian receiver hardware: bakelite dials, a swinging needle, a small round cathode screen flickering with static; cool moonlight on metal, warm glow from the dials. No people.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Macro on Martian receiver hardware: bakelite dials, a swinging needle, a small round cathode screen flickering with static; cool moonlight on metal, warm glow from the dials. No people. The needle trembles and the static flickers. Locked off with the slightest push. Camera and motion: locked off on a tripod, the frame never moves; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `astronaut_close_sync`  ·  concept, performance  ·  Astronaut close-up

Used by 4 cut(s): s012 (5.7s), s014 (2.9s), s026 (2.8s), s031 (2.8s)
Sections: mars-unmasking, scene-05 · **generate ≥ 5.7s**
Lyric: *I don't see anything. / Think you forgot to attach it. / If you've got it, give it.*
Conventions: `CAM-LOCKED`, `PERF-HYSTERETIC-PRESENCE`, `PERF-EXAGGERATED-ENUNCIATION`

**Still**

```
Film still. Frontal close-up of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned, head and collar ring, against a soft dark neutral backdrop, eyes locked on the lens, mouth open mid-phrase, deadpan self-assurance.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Frontal close-up of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned, head and collar ring, against a soft dark neutral backdrop, eyes locked on the lens, mouth open mid-phrase, deadpan self-assurance. She sings straight to camera with confident, punchy delivery: words clearly articulated, eyes locked on the lens, head steady, a subtle groove in her shoulders. Locked off. Camera and motion: locked off on a tripod, the frame never moves; she rises fast with the voice, holds across the gaps, and only slowly gives the phrase up; the mouth and the head lean overshoot on the strong syllables, then damp.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `mars_night_wide`  ·  concept  ·  Waiting on Mars, wide

Used by 1 cut(s): s013 (5.7s)
Sections: scene-05 · **generate ≥ 5.7s**
Lyric: *Don't clean it up first. / I'll work on it now.*
Conventions: `CAM-APPROACH`, `FRAME-NEGATIVE-WAKE`, `ANIM-MOVING-HOLD`

**Still**

```
Film still. Night. Symmetrical wide: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned stands small at dead center beside a glowing receiver mast, coral terrain silvered by two small moons and the striped planet, the colony domes dark on the horizon and the river a dark ribbon below. No one else in frame. Framing: a deliberate empty space at the center that the surrounding shapes bend around.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Symmetrical wide: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned stands small at dead center beside a glowing receiver mast, coral terrain silvered by two small moons and the striped planet, the colony domes dark on the horizon and the river a dark ribbon below. No one else in frame. Nothing moves but her breath and the mast's slow blink. Very slow push toward her. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `interference_bloom`  ·  concept  ·  Interference blooms

Used by 1 cut(s): s015 (2.9s)
Sections: scene-06 · **generate ≥ 2.9s**
Lyric: *In the garden, meanwhile—*
Conventions: `FRAME-CENTRIPETAL`, `GROW-PROPAGATION-WAVE`, `SONIC-PHRASE-COLOR`

**Still**

```
Film still. Macro of the receiver's round cathode screen: static blooming into concentric interference rings that resolve toward a green, helix-like pattern, the screen's green-gold glow spreading to the frame edges. No people. Framing: curves, gazes and paths all leading inward to one central event.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Macro of the receiver's round cathode screen: static blooming into concentric interference rings that resolve toward a green, helix-like pattern, the screen's green-gold glow spreading to the frame edges. No people. The rings resolve into leaves and the glow spreads until it fills the frame. Camera and motion: the change travels outward from a source, near things first, farther things later; the light changes color at the phrase boundaries, not continuously.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_threshold`  ·  concept  ·  Garden hatch opens

Used by 1 cut(s): s016 (2.9s)
Sections: scene-06 · **generate ≥ 2.9s**
Conventions: `CAM-APPROACH`, `FRAME-CENTRIPETAL`, `DEPTH-SCALE-VELOCITY`

**Still**

```
Film still. One-point symmetrical corridor: a riveted brass garden hatch centered at the end of a metal walkway, standing open onto the river forest - giant fern-fans, cupped leaves, pink thistle-globes - with green and magenta light spilling toward camera. No people. Framing: curves, gazes and paths all leading inward to one central event.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
One-point symmetrical corridor: a riveted brass garden hatch centered at the end of a metal walkway, standing open onto the river forest - giant fern-fans, cupped leaves, pink thistle-globes - with green and magenta light spilling toward camera. No people. Slow push through the hatch into the glow. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; near objects swell faster than far ones as the camera closes.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_walk_track`  ·  concept  ·  Forest walk, following the green light

Used by 2 cut(s): s017 (2.8s), s020 (2.8s)
Sections: psychedelic-garden · **generate ≥ 2.8s**
Lyric: *The-the-the-the-the / There's a little green light moving*
Conventions: `CAM-PROCESSION`, `ANIM-MULTIPLANE`, `DEPTH-MOTION-PARALLAX`

**Still**

```
Film still. Side-on planimetric view: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit, carrying a small brass hand lantern walks left to right along a mossy path through the river forest - towering fern-fans, giant cupped leaves, pink thistle-globes on tall stalks, hanging seed-lanterns, mist, layer after layer of foliage - while a small green light floats ahead of her at shoulder height. The canopy closes overhead; the light is green and magenta. She is centered, unhurried and deadpan. No one else in frame. Framing: foreground, midground and background clearly separated as distinct planes.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Side-on planimetric view: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit, carrying a small brass hand lantern walks left to right along a mossy path through the river forest - towering fern-fans, giant cupped leaves, pink thistle-globes on tall stalks, hanging seed-lanterns, mist, layer after layer of foliage - while a small green light floats ahead of her at shoulder height. The canopy closes overhead; the light is green and magenta. She is centered, unhurried and deadpan. No one else in frame. She follows the green light; slow 90-degree lateral track keeping her centered as the foliage layers slide past in deep parallax. Camera and motion: a lateral tracking shot, new elements entering at one frame edge as the last leave the other; the planes move at unequal rates as the camera travels; near layers slide faster than far layers.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_fauna_moth`  ·  concept  ·  Lantern-moth on a thistle-globe

Used by 1 cut(s): s018 (2.8s)
Sections: psychedelic-garden · **generate ≥ 2.8s**
Lyric: *through the psychedelic garden.*
Conventions: `FRAME-SCALE-BILLING`, `ANIM-MOVING-HOLD`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Macro in the river forest: a moth the size of a dinner plate resting on a pink thistle-globe, its four glassy wings lit from within like paper lanterns, antennae like brass filigree, fine fur on its body; foliage soft behind. Hand-built practical creature. No people. Framing: the lead largest and nearest, everything else smaller and farther.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Macro in the river forest: a moth the size of a dinner plate resting on a pink thistle-globe, its four glassy wings lit from within like paper lanterns, antennae like brass filigree, fine fur on its body; foliage soft behind. Hand-built practical creature. No people. The wings open and close once, slowly, their inner light pulsing on the beat; antennae quiver. Locked off. Camera and motion: the lead advances and grows on the strong syllables and eases back on the breath; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_fauna_eyes`  ·  concept  ·  Eyes in the foliage

Used by 1 cut(s): s019 (2.8s)
Sections: psychedelic-garden · **generate ≥ 2.8s**
Lyric: *Something with no name is waking*
Conventions: `FRAME-NEGATIVE-WAKE`, `DEPTH-RELATIVE-SCALE`, `ANIM-CYCLE-VARIATION`

**Still**

```
Film still. Night-dark corner of the river forest: several pairs of large round lantern-eyes at different depths and sizes among the giant leaves, each pair a different color, the creatures themselves only suggested by silhouette; the small green light passing between them. No people. Framing: a deliberate empty space at the center that the surrounding shapes bend around; the same motif repeated at several sizes into the distance.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night-dark corner of the river forest: several pairs of large round lantern-eyes at different depths and sizes among the giant leaves, each pair a different color, the creatures themselves only suggested by silhouette; the small green light passing between them. No people. The eyes blink at different times, one pair per beat, and turn to follow the green light as it passes. Locked off. Camera and motion: the repeated action never repeats exactly: a different entry edge, arc, scale or phase each time.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_fauna_birds`  ·  concept  ·  Chrome hummingbird-things at the seed-lanterns

Used by 1 cut(s): s021 (2.8s)
Sections: psychedelic-garden · **generate ≥ 2.8s**
Lyric: *And whatever woke is waiting*
Conventions: `FRAME-PERIPHERAL-COUNTERPOINT`, `ANIM-CYCLE-VARIATION`, `DEPTH-MOTION-PARALLAX`

**Still**

```
Film still. Medium in the river forest canopy: a small flock of chrome hummingbird-things, each no bigger than a thumb with a needle beak and a fan of glass tail feathers, hovering at a cluster of hanging seed-lanterns, layered in depth. Hand-built practical creatures. No people. Framing: independent secondary life at the frame edges, partly hidden.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Medium in the river forest canopy: a small flock of chrome hummingbird-things, each no bigger than a thumb with a needle beak and a fan of glass tail feathers, hovering at a cluster of hanging seed-lanterns, layered in depth. Hand-built practical creatures. No people. They dart from lantern to lantern on the drum hits, each on its own arc, never in unison, near ones crossing the lens. Locked off. Camera and motion: background action continuing on its own arcs at the edges, never in step with the lead; the repeated action never repeats exactly: a different entry edge, arc, scale or phase each time; near layers slide faster than far layers.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `forest_awakening_frontal`  ·  concept  ·  Forest wakes to drums

Used by 1 cut(s): s022 (2.8s)
Sections: psychedelic-garden · **generate ≥ 2.8s**
Lyric: *for the song to begin.*
Conventions: `CAM-LOCKED`, `GROW-PROPAGATION-WAVE`, `GROW-ENVIRONMENT-ANSWER`

**Still**

```
Film still. Frontal symmetrical wide: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit, carrying a small brass hand lantern stands dead center on the forest path, small under the canopy, as the whole forest lights up around her - cupped leaves snapping open, thistle-globes glowing pink row by row, seed-lanterns lit, spore puffs rising - a blazing symmetrical mandala of foliage. She holds still and deadpan. No one else in frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Frontal symmetrical wide: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit, carrying a small brass hand lantern stands dead center on the forest path, small under the canopy, as the whole forest lights up around her - cupped leaves snapping open, thistle-globes glowing pink row by row, seed-lanterns lit, spore puffs rising - a blazing symmetrical mandala of foliage. She holds still and deadpan. No one else in frame. The forest wakes in waves on the drum hits, row by row from the edges to the center. Locked off. Camera and motion: locked off on a tripod, the frame never moves; the change travels outward from a source, near things first, farther things later; different parts of the scene answer different instruments: drums trigger the leaves, bass moves the big roots, the voice opens the flowers.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `specimen_table_macro`  ·  concept  ·  Specimen table mandala

Used by 2 cut(s): s023 (5.7s), s025 (5.7s)
Sections: pattern-aperture · **generate ≥ 5.7s**
Lyric: *Okay, there. / You've got it. / That should be enough.*
Conventions: `CAM-APPROACH`, `PERF-RESPONSE-CHAIN`, `ANIM-SLOW-IN-OUT`

**Still**

```
Film still. Top-down overhead of a white enamel specimen table: silver-gloved hands arranging fragments of an alien specimen - translucent organic shards with helical structure inside, spiked seed pods, a chrome scale - into a circular diagram that almost completes a pattern, one piece sitting wrong. Only the hands are in frame; no people.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Top-down overhead of a white enamel specimen table: silver-gloved hands arranging fragments of an alien specimen - translucent organic shards with helical structure inside, spiked seed pods, a chrome scale - into a circular diagram that almost completes a pattern, one piece sitting wrong. Only the hands are in frame; no people. The hands try the wrong piece in every position. Slow overhead push. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; one thing initiates, another accommodates, the contact gives each an unequal recoil; moves ease in and ease out, quick through the middle, settling at the pose.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `astronaut_inspect_medium`  ·  performance  ·  Astronaut inspects specimen

Used by 1 cut(s): s024 (5.7s)
Sections: pattern-aperture · **generate ≥ 5.7s**
Lyric: *Just look and tell me. / Do you see a pattern? / I'm sure it's there.*
Conventions: `CAM-LOCKED`, `ANIM-MOVING-HOLD`, `FRAME-SCALE-BILLING`

**Still**

```
Film still. Waist-up medium of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned at an enamel field table at the forest's edge, holding a strange translucent organic specimen up to the light, a helical structure glowing faintly inside it in silver-gloved hands, unhurried and deadpan; instruments in soft focus behind. Symmetrical. No one else in frame. Framing: the lead largest and nearest, everything else smaller and farther.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Waist-up medium of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned at an enamel field table at the forest's edge, holding a strange translucent organic specimen up to the light, a helical structure glowing faintly inside it in silver-gloved hands, unhurried and deadpan; instruments in soft focus behind. Symmetrical. No one else in frame. She turns the specimen slowly in the light. Locked off. Camera and motion: locked off on a tripod, the frame never moves; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves; the lead advances and grows on the strong syllables and eases back on the breath.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `mars_domes_wide`  ·  concept  ·  Colony vista reveal

Used by 1 cut(s): s027 (2.8s)
Sections: mars-unmasking · **generate ≥ 2.8s**
Lyric: *You come back around. / I'll pull it into a shape*
Conventions: `CAM-APPROACH`, `ANIM-MULTIPLANE`, `DEPTH-RELATIVE-SCALE`

**Still**

```
Film still. Grand symmetrical vista: the cream-and-turquoise domed colony with needle towers under a mint-green sky, a winding river of blue-green water snaking up the center of frame through coral terrain to the wide mirror-calm lagoon at its foot, striped planet above. Miniature-set depth. No people. Framing: foreground, midground and background clearly separated as distinct planes; the same motif repeated at several sizes into the distance.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Grand symmetrical vista: the cream-and-turquoise domed colony with needle towers under a mint-green sky, a winding river of blue-green water snaking up the center of frame through coral terrain to the wide mirror-calm lagoon at its foot, striped planet above. Miniature-set depth. No people. Very slow push along the river toward the colony; the water shimmers gently, nothing else moves; every dome and tower keeps its shape; no people appear. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; the planes move at unequal rates as the camera travels.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `astronaut_mars_medium`  ·  performance  ·  Astronaut above colony

Used by 1 cut(s): s028 (2.8s)
Sections: mars-unmasking · **generate ≥ 2.8s**
Lyric: *and spin your world around. / No. / Don't make it better.*
Conventions: `CAM-SUSTAINED-REFRAME`, `PERF-ARC`, `ANIM-FOLLOW-THROUGH`

**Still**

```
Film still. Medium shot: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned centered on a coral ridge above the domed colony and its river lagoon, vista soft behind her, facing camera, wind in her hair. No one else in frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Medium shot: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned centered on a coral ridge above the domed colony and its river lagoon, vista soft behind her, facing camera, wind in her hair. No one else in frame. She turns from the view to face camera; slow lateral track that settles frontal and locked. Camera and motion: one continuous slow reframe from a readable anchor to a second anchor, easing across the space while the action continues; every move travels on a curved line of action with a slight overshoot and recovery; trailing parts overshoot and return after the main mass has stopped.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `counterfeit_parade`  ·  concept  ·  Parade of fakes

Used by 1 cut(s): s029 (2.8s)
Sections: mars-unmasking · **generate ≥ 2.8s**
Lyric: *Don't want fake shit.*
Conventions: `CAM-LOCKED`, `CAM-PROCESSION`, `FRAME-CENTRIFUGAL`

**Still**

```
Film still. Deadpan planimetric side-on view: a procession of too-perfect objects - polished miniature domes, smiling masks, trophy-bright Mars globes - lined up left to right across frame on a hidden conveyor; the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned stands centered behind it, unimpressed. No one else in frame. Framing: major shapes cropped by the frame edges, the action plainly continuing beyond the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Deadpan planimetric side-on view: a procession of too-perfect objects - polished miniature domes, smiling masks, trophy-bright Mars globes - lined up left to right across frame on a hidden conveyor; the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned stands centered behind it, unimpressed. No one else in frame. The procession glides left to right. Locked off. Camera and motion: locked off on a tripod, the frame never moves; a lateral tracking shot, new elements entering at one frame edge as the last leave the other; elements keep their trajectories before entering and after leaving the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `excavation_site`  ·  performance  ·  Digging the real thing

Used by 1 cut(s): s030 (2.8s)
Sections: mars-unmasking · **generate ≥ 2.8s**
Lyric: *Dig the thing underneath it. / Dig it stupid.*
Conventions: `CAM-APPROACH`, `PERF-ANTICIPATION`, `ANIM-FOLLOW-THROUGH`

**Still**

```
Film still. Low frontal shot: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned kneels in coral soil, lifting a clumsy, handmade, unpolished object out of the ground with silver gloves - clearly loved; dust motes in warm light. No one else in frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Low frontal shot: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned kneels in coral soil, lifting a clumsy, handmade, unpolished object out of the ground with silver gloves - clearly loved; dust motes in warm light. No one else in frame. She digs and lifts it free. Slight slow push toward her hands. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; a small contrary preparation before each accent: a draw-back, a compression, a look; trailing parts overshoot and return after the main mass has stopped.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `weather_over_colony`  ·  concept  ·  Filament weather

Used by 1 cut(s): s032 (2.8s)
Sections: semantic-rocket-weather · **generate ≥ 2.8s**
Lyric: *Can you hear / what I meant? / Sometimes.*
Conventions: `CAM-ASCENT`, `FRAME-CENTRIFUGAL`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Wide sky over the domes and the river lagoon: strange luminous filament weather - slow silk-like streamers of pastel light peeling off cloud fronts and drifting apart, no lightning, no flashes. No people. Framing: major shapes cropped by the frame edges, the action plainly continuing beyond the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Wide sky over the domes and the river lagoon: strange luminous filament weather - slow silk-like streamers of pastel light peeling off cloud fronts and drifting apart, no lightning, no flashes. No people. The pastel streamers peel slowly off the cloud fronts and drift apart across the sky while the clouds creep; the domes and the water stay still; no lightning, no flashes, no people appear. Slow upward drift. Camera and motion: a crane rise through the layers, near things dropping away until the individuals read as one formation; elements keep their trajectories before entering and after leaving the frame; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `duet_call_response_sync`  ·  performance  ·  Split duet call/response

Used by 2 cut(s): s033 (2.9s), s034 (2.9s)
Sections: semantic-rocket-weather · **generate ≥ 2.9s**
Lyric: *Not always. / Meaning leaves thread.*
Conventions: `CAM-LOCKED`, `CAM-ATTENTION-HANDOFF`, `PERF-HYSTERETIC-PRESENCE`

**Still**

```
Film still. Symmetrical split staging divided by a thin centered seam of static: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt in her warm-gold study at frame left, the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned in cool-teal Martian night at frame right, the same face mirrored, both facing the lens mid-phrase. No one else in frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Symmetrical split staging divided by a thin centered seam of static: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt in her warm-gold study at frame left, the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned in cool-teal Martian night at frame right, the same face mirrored, both facing the lens mid-phrase. No one else in frame. They trade lines like a confident call-and-response, clearly articulated, heads steady, each landing her phrase on the beat. Locked off. Camera and motion: locked off on a tripod, the frame never moves; a rack focus that passes attention from one depth to another, the first softening as the second sharpens; she rises fast with the voice, holds across the gaps, and only slowly gives the phrase up.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `orbit_rockets_descent`  ·  concept  ·  Rockets descend from orbit

Used by 1 cut(s): s035 (2.9s)
Sections: semantic-rocket-weather · **generate ≥ 2.9s**
Lyric: *Things clap overhead*
Conventions: `CAM-LOCKED`, `DEPTH-RELATIVE-SCALE`, `ANIM-SLOW-IN-OUT`

**Still**

```
Film still. From orbit: the coral globe of Mars fills the lower frame, threaded with winding blue-green rivers and dark oases, its terminator curving away; three finned chrome retro rockets descend toward it in slow formation, tail-first, retro-flames flickering; the striped gas planet and small moons beyond. No people. Framing: the same motif repeated at several sizes into the distance.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
From orbit: the coral globe of Mars fills the lower frame, threaded with winding blue-green rivers and dark oases, its terminator curving away; three finned chrome retro rockets descend toward it in slow formation, tail-first, retro-flames flickering; the striped gas planet and small moons beyond. No people. The formation sinks toward the planet, flames pulsing. Locked off in space, symmetrical. Camera and motion: locked off on a tripod, the frame never moves; moves ease in and ease out, quick through the middle, settling at the pose.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `rockets_landing_wide`  ·  concept  ·  Rockets land

Used by 1 cut(s): s036 (2.9s)
Sections: semantic-rocket-weather · **generate ≥ 2.9s**
Lyric: *like rockets coming in.*
Conventions: `CAM-LOCKED`, `ANIM-IMPACT-ACCENT`, `FRAME-SCALE-BILLING`

**Still**

```
Film still. Wide symmetrical landing field at the edge of the colony above the river: three finned chrome rockets settling onto brass landing rings in a row, tail-first on columns of flame, rings of coral dust rolling outward; the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit with a clear domed glass helmet sealed to the collar ring stands small at dead center in the foreground, facing camera, the rockets landing behind her. No one else in frame. Framing: the lead largest and nearest, everything else smaller and farther.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Wide symmetrical landing field at the edge of the colony above the river: three finned chrome rockets settling onto brass landing rings in a row, tail-first on columns of flame, rings of coral dust rolling outward; the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit with a clear domed glass helmet sealed to the collar ring stands small at dead center in the foreground, facing camera, the rockets landing behind her. No one else in frame. The rockets touch down one after another on the beat and the dust rolls past her; no one else appears. Locked off. Camera and motion: locked off on a tripod, the frame never moves; one hard accent drawn as a single-frame flare, flattening or starburst on the hit; the lead advances and grows on the strong syllables and eases back on the breath.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `rain_to_canal`  ·  concept  ·  Rain becomes river

Used by 2 cut(s): s037 (2.9s), s038 (2.9s)
Sections: semantic-rocket-weather · **generate ≥ 2.9s**
Lyric: *Can't tell you / if they're true.*
Conventions: `CAM-SUSTAINED-REFRAME`, `GROW-PROPAGATION-WAVE`, `EDIT-GRAPHIC-MATCH`

**Still**

```
Film still. Rain over red terrain: falling water gathering into bright rivulets that join and wind together into a young river snaking toward the horizon between coral banks. No people. Framing: a strong central contour that the next shot can match.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Rain over red terrain: falling water gathering into bright rivulets that join and wind together into a young river snaking toward the horizon between coral banks. No people. Slow tilt and track following the water downhill as it winds, ending level on the moving current. Camera and motion: one continuous slow reframe from a readable anchor to a second anchor, easing across the space while the action continues; the change travels outward from a source, near things first, farther things later; the move ends on the contour the next shot begins with.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `basin_moon_descent`  ·  concept  ·  Descent to the lagoon

Used by 2 cut(s): s039 (2.9s), s040 (2.9s)
Sections: rivers-of-mars · **generate ≥ 2.9s**
Lyric: *Elsewhere, under a smaller moon—*
Conventions: `CAM-DESCENT`, `FRAME-PATTERN-SURRENDER`, `DEPTH-SCALE-VELOCITY`

**Still**

```
Film still. Night. From high above the colony's river lagoon: the reflection of a small warm moon dead center in a wide mirror-calm expanse of dark water held by a rocky coral shore, the colony's fountains along the far bank; the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison glinting beneath the surface, arranged in concentric rings. No people. Framing: many identical performers arranged into one larger figure: a ring, a spiral, a fan.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. From high above the colony's river lagoon: the reflection of a small warm moon dead center in a wide mirror-calm expanse of dark water held by a rocky coral shore, the colony's fountains along the far bank; the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison glinting beneath the surface, arranged in concentric rings. No people. The camera cranes slowly down toward the water; the rings resolve as it nears the surface. Camera and motion: a plunge toward an aperture, foreshortening accelerating until the opening expands past the frame; the individuals lock into the pattern on the accent and break out of it after; near objects swell faster than far ones as the camera closes.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `basin_edge_sync`  ·  performance  ·  Astronaut at the lagoon shore

Used by 2 cut(s): s041 (2.8s), s042 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *in the rivers of Mars.*
Conventions: `CAM-BREATH`, `PERF-HYSTERETIC-PRESENCE`, `FRAME-SCALE-BILLING`

**Still**

```
Film still. Night. the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned kneels at the rocky shore of the wide river lagoon in moonlight, lamp beside her, facing the lens mid-phrase; behind her the open water is wide and calm, and one of the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison has just surfaced. No one else in frame. Symmetrical framing on her and her reflection. Framing: the lead largest and nearest, everything else smaller and farther.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned kneels at the rocky shore of the wide river lagoon in moonlight, lamp beside her, facing the lens mid-phrase; behind her the open water is wide and calm, and one of the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison has just surfaced. No one else in frame. Symmetrical framing on her and her reflection. She sings with confident, rhythmic delivery, head steady, shoulders grooving subtly; the swimmer bobs on the beat. Slow push. Camera and motion: a damped push and release with the phrase: a small forward lean on the articulation, a slower settle on the breath; she rises fast with the voice, holds across the gaps, and only slowly gives the phrase up; the lead advances and grows on the strong syllables and eases back on the breath.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `basin_kaleidoscope_top`  ·  concept  ·  Water ballet from above

Used by 2 cut(s): s043 (2.8s), s047 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *in the rivers of Mars.*
Conventions: `CAM-LOCKED`, `FRAME-PATTERN-SURRENDER`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Night. Straight-down overhead of the wide river lagoon lit like a stage: the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison arranged in concentric rings and radiating spokes across the open water, tails fanning outward in unison so the whole school reads as one chrome flower; the colony's fountains firing along the rocky shore; the moon's reflection dead center. Busby Berkeley water-ballet geometry. No people. Framing: many identical performers arranged into one larger figure: a ring, a spiral, a fan.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Straight-down overhead of the wide river lagoon lit like a stage: the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison arranged in concentric rings and radiating spokes across the open water, tails fanning outward in unison so the whole school reads as one chrome flower; the colony's fountains firing along the rocky shore; the moon's reflection dead center. Busby Berkeley water-ballet geometry. No people. The rings rotate against each other and the tails fan open and closed on the beat. Locked overhead. Camera and motion: locked off on a tripod, the frame never moves; the individuals lock into the pattern on the accent and break out of it after; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `merfolk_surface_ring`  ·  concept  ·  Tiny mouths agape

Used by 1 cut(s): s044 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *Tails wagging to music*
Conventions: `CAM-LOCKED`, `FRAME-PATTERN-SURRENDER`, `ANIM-ANTICIPATION`

**Still**

```
Film still. Night. Waterline shot at the center of the wide river lagoon: a ring of the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison surfacing in unison, chrome heads risen together, single lashed eyes open, tiny round mouths open in song, tails fanned behind them; concentric ripples spreading outward across the open water, the colony domes dark on the far shore. Symmetrical, locked at water level. No people. Framing: many identical performers arranged into one larger figure: a ring, a spiral, a fan.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Waterline shot at the center of the wide river lagoon: a ring of the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison surfacing in unison, chrome heads risen together, single lashed eyes open, tiny round mouths open in song, tails fanned behind them; concentric ripples spreading outward across the open water, the colony domes dark on the far shore. Symmetrical, locked at water level. No people. They rise together on the downbeat, blink, and sing; the ripples spread. Locked off. Camera and motion: locked off on a tripod, the frame never moves; the individuals lock into the pattern on the accent and break out of it after; a brief lean or compression opposite the coming move.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `chorus_close_sync`  ·  performance  ·  Chorus hero close-up

Used by 2 cut(s): s045 (2.8s), s046 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *that bends your mind.*
Conventions: `CAM-LOCKED`, `PERF-EXAGGERATED-ENUNCIATION`, `PERF-HYSTERETIC-PRESENCE`

**Still**

```
Film still. Night. Frontal hero close-up of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned at the lagoon shore, moonlit, ripples of reflected water-light across her face, eyes locked on the lens, mouth open mid-belt, confident and punchy.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Frontal hero close-up of the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit: a plain, smooth, close-fitting one-piece brushed-silver spacesuit with a single wide riveted silver collar ring and matching silver gloves and boots, unadorned at the lagoon shore, moonlit, ripples of reflected water-light across her face, eyes locked on the lens, mouth open mid-belt, confident and punchy. She belts the chorus hook straight to camera - words clearly articulated, head steady, shoulders moving subtly with the groove; the water-light pulses on the beat. Locked off. Camera and motion: locked off on a tripod, the frame never moves; the mouth and the head lean overshoot on the strong syllables, then damp; she rises fast with the voice, holds across the gaps, and only slowly gives the phrase up.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `drumwall_basin`  ·  concept  ·  Fountains on the beat

Used by 1 cut(s): s048 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *Whatever falls from heaven*
Conventions: `CAM-LOCKED`, `ANIM-IMPACT-ACCENT`, `GROW-PROPAGATION-WAVE`

**Still**

```
Film still. Night. One-point symmetrical view across the river lagoon toward the colony: ranked fountains along the far shore firing hard, jets punching up in sequence, lit warm against the dark domes; the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison surfacing in a line across the water on the beat; the water otherwise calm and open. No people.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. One-point symmetrical view across the river lagoon toward the colony: ranked fountains along the far shore firing hard, jets punching up in sequence, lit warm against the dark domes; the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison surfacing in a line across the water on the beat; the water otherwise calm and open. No people. The fountains fire on the drum hits, in sequence along the shore, and the mer-folk rise and dip in time; no people appear. Locked off while the water performs. Camera and motion: locked off on a tripod, the frame never moves; one hard accent drawn as a single-frame flare, flattening or starburst on the hit; the change travels outward from a source, near things first, farther things later.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `canal_current_macro`  ·  concept  ·  Inside the current

Used by 2 cut(s): s049 (2.8s), s053 (2.9s)
Sections: current-turns-home, rivers-of-mars · **generate ≥ 2.9s**
Lyric: *leaves the senses behind.*
Conventions: `CAM-PROCESSION`, `DEPTH-MOTION-PARALLAX`, `FRAME-CENTRIFUGAL`

**Still**

```
Film still. Underwater inside the river current: the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison passing through shafts of moonlight in loose formation, bubbles and bright motes in deep parallax, ribbons of reflected light bending across them. No people. Framing: major shapes cropped by the frame edges, the action plainly continuing beyond the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Underwater inside the river current: the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison passing through shafts of moonlight in loose formation, bubbles and bright motes in deep parallax, ribbons of reflected light bending across them. No people. Slow lateral drift; the school never restarts, it just keeps passing, the light-ribbons bending with the music. Camera and motion: a lateral tracking shot, new elements entering at one frame edge as the last leave the other; near layers slide faster than far layers; elements keep their trajectories before entering and after leaving the frame.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `chorus_plunge`  ·  concept  ·  Plunge into the basin

Used by 2 cut(s): s050 (2.8s), s051 (2.8s)
Sections: rivers-of-mars · **generate ≥ 2.8s**
Lyric: *No one has ever been*
Conventions: `CAM-DESCENT`, `FRAME-LENS-CROSSING`, `DEPTH-SCALE-VELOCITY`

**Still**

```
Film still. Single frame, no panels: looking straight down at the surface of the wide river lagoon from a few feet above it, moonlit spray and bubbles bursting upward, the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison visible scattering beneath the surface. No people. Framing: a large foreground object very close to the lens, partly occluding the scene.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Single frame, no panels: looking straight down at the surface of the wide river lagoon from a few feet above it, moonlit spray and bubbles bursting upward, the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison visible scattering beneath the surface. No people. One smooth accelerating push straight down through the surface: spray, bubbles, then the school scattering and reforming around the camera underwater. One continuous move, no cut. Camera and motion: a plunge toward an aperture, foreshortening accelerating until the opening expands past the frame; a foreground object sweeps close past the lens, occludes, and exits an edge; near objects swell faster than far ones as the camera closes.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `canal_junction_turn`  ·  concept  ·  The current turns in the eddy

Used by 1 cut(s): s052 (2.8s)
Sections: current-turns-home · **generate ≥ 2.8s**
Lyric: *Then turns.*
Conventions: `CAM-LOCKED`, `FRAME-PATTERN-SURRENDER`, `PERF-ARC`

**Still**

```
Film still. Top-down planimetric view of the great slow eddy where the river enters the lagoon: the current curling into a wide spiral, the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison wheeling around together in the turning water to head back the way they came. No gates, no machinery, no people. Framing: many identical performers arranged into one larger figure: a ring, a spiral, a fan.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Top-down planimetric view of the great slow eddy where the river enters the lagoon: the current curling into a wide spiral, the river mer-folk - a school of chrome-plated swimmers polished like 1950s hood ornaments, all identical: each has a long fish tail, slender humanoid arms, and a smooth bald chrome head with exactly one large round eye set in the centre of the face with long lashes, no second eye, no nose, and a tiny round mouth open in song - moving in unison wheeling around together in the turning water to head back the way they came. No gates, no machinery, no people. The eddy turns on the beat, the current visibly reverses, and the school wheels as one; no people appear. Locked overhead. Camera and motion: locked off on a tripod, the frame never moves; the individuals lock into the pattern on the accent and break out of it after; every move travels on a curved line of action with a slight overshoot and recovery.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `duet_lamplight_astronomer_sync`  ·  performance  ·  Astronomer lamplight outro

Used by 2 cut(s): s054 (5.7s), s057 (5.9s)
Sections: low-light-outro, scene-14 · **generate ≥ 5.9s**
Lyric: *You still there? / Define there.*
Conventions: `CAM-LOCKED`, `ANIM-MOVING-HOLD`, `PERF-ABSENCE`

**Still**

```
Film still. Night interior close-medium: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt in the small pool of her desk lamp, a few curls escaped from the twist, surrounded by darkness, tired and fond, facing the lens. Centered.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night interior close-medium: the astronomer, the astronaut's identical twin sister - the same heart-shaped face, high cheekbones, large wide-set hazel-green eyes, small straight nose, full mouth and light freckles, the same ballerina's build and dancer's carriage, the same auburn hair pinned up in a loose 1950s French twist with straight bangs and a pencil pushed through it - in a rumpled rust-brown tweed jacket, mustard knit vest and collared check shirt in the small pool of her desk lamp, a few curls escaped from the twist, surrounded by darkness, tired and fond, facing the lens. Centered. She sings the quiet lines to camera with barely moving lips. Locked off. Camera and motion: locked off on a tripod, the frame never moves; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves; on the breath the performer retreats and another layer inherits the attention.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `duet_lamplight_astronaut_sync`  ·  performance  ·  Astronaut lamplight outro

Used by 1 cut(s): s055 (5.7s)
Sections: low-light-outro · **generate ≥ 5.7s**
Lyric: *Never mind. / La-la-la, low light. / No, go on. / Thought I heard something.*
Conventions: `CAM-LOCKED`, `ANIM-MOVING-HOLD`, `PERF-ABSENCE`

**Still**

```
Film still. Night interior close-medium, the exact mirror of the astronomer's lamplight frame: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit with the collar ring removed, a plain open neckline in her bunk under a small warm lamp, cool Martian night at the porthole behind, tired and fond, facing the lens. Centered.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night interior close-medium, the exact mirror of the astronomer's lamplight frame: the astronaut, a young woman with a ballerina's build - slender, long-necked, long-limbed - and a dancer's carriage, upright with shoulders back and chin lifted; a heart-shaped face with high cheekbones, large wide-set hazel-green eyes under arched brows, a small straight nose, a full expressive mouth, fair skin with light freckles; an auburn curly chin-length bob with straight bangs in her silver suit with the collar ring removed, a plain open neckline in her bunk under a small warm lamp, cool Martian night at the porthole behind, tired and fond, facing the lens. Centered. She sings softly to camera with minimal motion. Locked off. Camera and motion: locked off on a tripod, the frame never moves; a held pose with internal life: breath, gaze, a strand of hair, one hand; nothing else moves; on the breath the performer retreats and another layer inherits the attention.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `telescope_time_tunnel`  ·  concept  ·  Telescope time tunnel

Used by 1 cut(s): s056 (5.8s)
Sections: scene-14 · **generate ≥ 5.8s**
Lyric: *You did. / Was it you? / Da-da-dum. / Could've been. / Say goodnight. / Who?*
Conventions: `CAM-APPROACH`, `FRAME-CHROMATIC-TUNNEL`, `SONIC-MASS-FOLLOWER`

**Still**

```
Film still. Looking down the interior of the great telescope tube: concentric brass rings receding to a far point of starlight, perfectly centered. No people. Framing: concentric colored bands and radial planes filling the frame as the stage itself.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Looking down the interior of the great telescope tube: concentric brass rings receding to a far point of starlight, perfectly centered. No people. The starlight pulses gently with the rhythm; slow steady push into the rings. Camera and motion: a slow dolly-in that gathers speed, foreground parallax quickening so the subject grows faster than the background: travel, not a zoom; the bands pulse in depth with the percussion; the big slow things carry the weight of the music: size, glow and mass move slowly, only the fine edges are fast.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `colony_goodnight_cascade`  ·  concept  ·  Colony goodnight

Used by 1 cut(s): s058 (3.0s)
Sections: scene-14 · **generate ≥ 3.0s**
Lyric: *Night.*
Conventions: `CAM-LOCKED`, `GROW-PROPAGATION-WAVE`, `FRAME-NEGATIVE-WAKE`

**Still**

```
Film still. Night. Wide shot of the colony from across the river lagoon: most dome windows and tower lights already dark, a last few still lit down the valley, the water holding a single moon reflection. No people. Framing: a deliberate empty space at the center that the surrounding shapes bend around.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. Wide shot of the colony from across the river lagoon: most dome windows and tower lights already dark, a last few still lit down the valley, the water holding a single moon reflection. No people. The remaining lights switch off in cascading order, one per beat, until only the moon's reflection is left. Locked off. Camera and motion: locked off on a tripod, the frame never moves; the change travels outward from a source, near things first, farther things later.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

## `night_iris_title`  ·  concept  ·  Night iris out

Used by 1 cut(s): s059 (5.9s)
Sections: production-coda · **generate ≥ 5.9s**
Conventions: `CAM-LOCKED`, `FRAME-CENTRIPETAL`, `ANIM-SLOW-IN-OUT`

**Still**

```
Film still. Night. The final wide frame of the colony darkening at its edges, a circular iris closing in on the moon's reflection in the lagoon. No people. Framing: curves, gazes and paths all leading inward to one central event.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```

**Video**

```
Night. The final wide frame of the colony darkening at its edges, a circular iris closing in on the moon's reflection in the lagoon. No people. The iris closes until only that small light remains, then black. (Title is added in post, never generated.) Camera and motion: locked off on a tripod, the frame never moves; moves ease in and ease out, quick through the middle, settling at the pose.

Live-action 1950s Technicolor science-fiction feature, Wes Anderson style:
planimetric symmetrical composition, centered subjects, flat frontal staging,
deadpan performance. Photographed on 35mm three-strip Technicolor: real
actors, practical sets, miniatures and painted backdrops — never an
illustration, painting, cartoon or render. Soundstage Mars with
matte-painting depth: coral-red terrain, winding natural rivers of
blue-green water with settlements built up along their banks, cream and
turquoise domed buildings, needle towers, spiked ornamental
plants, striped gas planet and small moons in a mint-green sky. One riveted silver spacesuit, brass and bakelite instruments, hand-built
practical props. The only figures that ever appear are the two twin sisters
and the chrome mer-folk; no other people, no extras, no spacemen. Soft even studio lighting, saturated pastel palette. Reflected light
may move across faces and costumes but never changes the surface itself: no
glitter, no scales, no sequins forming on skin or fabric. Camera locked off
or moving in slow symmetrical push-ins and 90-degree lateral tracks; no
handheld, no zooms, no whip pans, no speed ramps. Consistent cast, wardrobe
and hair throughout. No text, no titles, no captions, no logos, no
watermarks.
```
